...
- Create feature branch from feature branch: `git checkout feature/feature-{id} && git checkout -b feature/wi-{id}-{slug}`
- Commits: `[hive:dev-N] WI-{id}: {description}`
- Rebase onto the **feature branch** (`feature/feature-{id}`) before submitting for review.
