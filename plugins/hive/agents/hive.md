...
### Timestamps
All timestamps in logs and state files are generated by hook scripts using the `date` command for actual system time. Agents NEVER write timestamps manually -- not in logs, state files, or commit messages.
