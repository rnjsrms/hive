...
6. **Feature End** -- When all WIs are MERGED to the feature branch:
   ...
   2. **Create PR**: `gh pr create --base {base_branch} --head feature-{id} --title '[hive] {feature_name} ({feature_id})' --body '{body}'`. The body includes a work items table (ID, title, status).
...
### Gitflow
- Feature branches: `feature-{id}` — created by lead from base_branch at feature start.
- WI branches: `{feature_branch}_wi-{id}` (e.g., `feature-1_wi-3`) — created by developers from feature branch. WI IDs restart at 1 per feature; the globally unique ID is `{feature_branch}_wi-{id}`.
...
- **Workers** own: their assigned `feature-{id}_wi-{id}.json` files (status and history fields only)
...
9. **Branches follow naming convention.** WI: `{feature_branch}_wi-{id}`. Feature: `feature-{id}`. No exceptions.
